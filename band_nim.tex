...
detect backward neutrons with momenta between $0.25$ and $0.6$ \si{\GeV/\clight}. It consists of $18$ rows and $5$ layers of $7.2$ \si{\centi\meter} 
...
The CLAS12 (Cebaf Large Angle Spectrometer)[ADD REF TO NIM] in Jefferson Lab's HallB experimental hall is a multi-purpose spectrometer to detect charged and neutral particles. It covers angles from xx to xx and momenta from xx to xx for the different particles. For our physics of interest, it is necessary to detect recoil neutrons with momenta above \SI{200/250}{\mega\eVperc} at backward angles. Therefore, a new detector has been build to detect these neutrons, the Backward Angle Neutron Detector (BAND). It is shown together with its surroundings in Fig.~\ref{fig:clas12} (top row). 
{\color{red} Shall we add more details about our physics here?} 
...
\begin{figure}[h]
	...
	\includegraphics[width=0.48\textwidth]{BAND_1-2.png}
		\caption{Top left: BAND and CLAS12. BAND is on the right side. Top right: BAND and its close surroundings: the target is shown in cyan,  BAND in red and the central detector region of CLAS12 in blue. Bottom row: BAND and its frame. The cyan color indicates the lead shield wall installed at the downstream side of BAND.}
		\label{fig:clas12}
\end{figure}

This newly build detector is designed to measure neutrons with an efficiency of $35$\%  and to cover backward angle from $155$\si{\degree} to $175$\si{\degree}. It is based on scintillator bars read-out on both sides with PMTs. 

In this paper, we first explain the general design of BAND and requirements on time resolutions  and geometry. The selection of the detector components are guided by lab measurements. This includes for example the selection of scintillant, PMTs or magnetic shielding. In the following chapter, we present the performance of the detector after its installtion in HallB of Jefferson Lab. A detailed overview of the necessary calibrations is given. Finally, we present obtained ToF resolutions and neutron-photon separation from  data obtained with an electron beam impinding on a deuterium target at Jefferson Lab.
...
Due to the placement of BAND in the fringe-field of the solenoid magnet of CLAS12 [CITE MAGNET NIM?], all PMTs need a magnetic shielding. Testing of possible materials was conducted at Old Dominion University. 
...
that can be used for monitoring performance stability and perform calibrations. A laser system [{\color{red}CITE THE LASER NIM}] was implemented to validate performance during data-taking. This system also 
...
From the splitter one signal is sent to flash-ADCs (250 VXS, 16 channels/board, made and owned by Jefferson Lab [ADD REF]) while the other signal is sent to  discriminators (16 channels/board, also made by Jefferson Lab [ADD REF, possibly online]).
The discriminated time signal goes to a TDC (CAEN VX1190A, 128 channels/board, 100 ps/channel resolution). 
In total, the system consists of 16 flash-ADCs in one VXS crate, 16 discriminators and a TDC in a VME crate and 16 splitters.  Furthermore, a trigger signal distribution card for the flash-ADCs and trigger interface boards are installed in the crates. All components are part of the standard electronics components of CLAS12 at Jefferson Lab[CITE NIM DAQ PAPER].
...
Time-walk calibrations were performed with the laser system [{\color{red}{CITE LASER NIM}}]. A motorized fiber optic attenuator is used to vary
