...
sizes of the scintillator bars and PMTs.  lots more detail about the
light guides here.  Show pictures and drawings.  Credit FSU.  mention the reflection problem???
...
and the left and right PMT energy peak locations for $^{90}$Sr and
  ...
  magnetic fields up to 80 G (limited by heat dissipation in the
  Helmholtz coils).
...
{\color{red} ADD PICTURE REF}.

As expected, the longitudinal magnetic field degraded PMT performance
...
1.5-\si{\milli\meter} $\mu$-metal.  The magnetic shields performed best when they
...
\begin{figure*}[tbh]
	...
	\caption{TO DO: UPDATE TEXTTime-walk calibration: Typical time to pulse height spectrum for one PMT with respect to the time for the reference photodiode before time-walk correction (left) and after time-walk correction (right).}
	\label{fig:time_walk}
\end{figure*}
