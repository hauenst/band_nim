...
\author{E.P.~Segarra, F.~Hauenstein\corref{corresponding}, A.~Schmidt, A.~Beck, \\ S.~May-Tal Beck, 
...
\author{P.~Eugenio}
\address{FSU}
\cortext[corresponding]{hauenst@jlab.org, Address: MIT Cambridge, Massachusetts 02139 
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\date{\today}
%\ead[]{}
...
\begin{figure*}[t!]
	\centering
		...
		\caption{BAND design	of active detector layers (left) and veto layer (right).  ({\color{red} Need an updated nicer figure with direction of beam / coordinate system, maybe having a darker/ligher shade of green for the short bars to distinguish between sectors})      }
		\label{fig:design}
\end{figure*}

The active detector area consists of 116 scintillator bars, arranged in 5 layers with 18 vertically stacked bars. The arrangement of the bars is shown in Fig. \ref{fig:design} (left). The bottom rows are only arranged in four layers due to obstructions from the cart below BAND.  Three different long bars are used in the detector: 15 bars with a length of $164\,\mathrm{cm}$ (red, Fig. \ref{fig:design}), 43 bars with a length of $202\,\mathrm{cm}$ (white, Fig. \ref{fig:design}) and 58 short bars with a length of $51\,\mathrm{cm}$ (cyan, Fig. \ref{fig:design}) to allow for the spared region for the beam pipe. All bars have light guides attached to both sides which are connected to either Hamamatsu R7724 \cite{pmtR7724} or ET 9214 \cite{pmt9214} 2-in PMTs for read-out.

{\color{red}TO DO: DEFINITION OF SECTORS AND LAYERS}
%
Table \ref{tab:geometry} summarizes the geometry parameters and PMTs for each sector.
%NOT PERFECT PHI SYMMETRY BUT NOT IMPORTANT FOR BAND PHYSICS
\begin{table*}[t]
\caption{{\color{red} THIS TABLE NEEDS AN UPDATE> IT IS JUST A TEMPORARY PLACEHOLDER WITH FTOF VALUES}Parameters for bars and PMTs for the different BAND sectors and layers.}
\begin{center}
...
A veto layer is installed with 24 scintillators bars which are read-out only on one side by ET 9954KB \cite{pmt9954} PMTs. The bars have a cross section of $2 \times 7.2\,\mathrm{cm}^{2}$. The bars in the veto layer have the same length as the bars of one layer of the active detector area. The veto layer is shown on the right in Fig. \ref{fig:design}.
...
\begin{figure}[h!]
	\centering
		\includegraphics[width=0.48\textwidth]{test-stand/all-lines-somedata.pdf}
		\caption{Obtained time resolution as a function of energy for various PMTs in the bench test measurements. {\color{red} ADD more description here}The combination shown in blue and red was choosen for the }
