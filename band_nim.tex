...
\begin{figure}[tb]
	\centering
		\includegraphics[width=0.44\textwidth]{pic-fiberonbar.pdf}
...
In Fig.~\ref{fig:final_offset} the time-of-flight spectrum is shown subtracted with the expected photon arrival time for each bar with a 2 \si{\mega\electronvolt} energy deposition cut to reduce random coincidence background. The red plot includes only time walk corrections but no offset corrections. One can see a broad distribution of the photon peak at $\approx$ 50 \si{\nano\s} far away from the expected zero value. This is expected since laser and beam-on data have different time delays from the trigger system of CLAS12. 
%Applying the individual offset corrections from the previous section, improves the photon peak but there are still residual and global offsets present.

Using the photon peak to correct for residual and global offsets for each bar results in the green spectrum which displays a well aligned photon peak with a width of $\approx$ 294 \si{\pico\s} below the design value.
Furthermore, they regular spikes in the remaining spectrum are photons correlated with off-time electrons from other beam bunches. The electron bunches are separated by 4 \si{\nano\s} which is consisted with the spacing of the spikes.
\begin{figure}[tb]
	\centering
		\includegraphics[width=0.48\textwidth]{time-alignment.pdf}
	\caption{Time-of-flight spectrum over all bars relative to the expected photon ToF after time-walk correction (red), %after time-walk and individual offset correction () 
	and after all corrections applied (green). All bars are well aligned with an averaged width of the photon peak of 294 \si{\pico\s}. A 2 \si{\mega\electronvolt} energy deposition cut is applied to reduce random coincidence background in the plots.}
...
\end{figure*}
%just temporary to correct for the footnotemark in the figure description for the first circular
\setcounter{footnote}{2}
The time-of-flight resolution of the detector are obtained by the laser calibration data and from the photon peak in data with beam. The laser data allows a scan of the time resolution over the whole energy deposition range (see Fig. \ref{fig:resolution-laser}, \footnotemark \footnotetext{NOTE FOR FIRST CIRCULAR: This plot will be updated with the results for layer 5 in the next days}) from 0.5 to 7 \si{\mega\electronvolt}. The resolutions are obtained from a Gaussian fit of the difference of the mean time of a bar to a reference bar as a function of ADC or energy deposit using the calibration from Section \ref{sec:energydeposit}). Therefore, one can compare resolutions for various energy deposition cuts. In Fig. \ref{fig:tof_resolution} (left), the ToF resolution for each bar is shown as a function of two deposition cuts, 1 \si{\mega\electronvolt} (blue) and 5 \si{\mega\electronvolt} (red). In 
...
The neutron identification for production data is done by cuts on time-of-flight per pathlength (ToF/m). Fig.~\ref{fig:tof} shows such a spectrum of events from production data where an electron was measured in coincidence in CLAS12 in the reaction $ed \rightarrow e'nX$. In addition, a minimum energy deposition cut of 5 \si{\MeV} has been applied for a BAND hit. One can clearly see two separated peaks for photons and neutrons over a flat background from accidentals. The dashed lines indicate the neutron of interest with momenta between $200$ and $600$ \si{\MeV/\clight}.

The minimum energy deposition was optimized using the neutron signal-to-background ratio, see Fig.~\ref{fig:signalbackground}. The signal region is defined as neutrons between $200$ and $600$ \si{\MeV\clight}, and the accidental background was estimated by fitting the constant edge to the left of the photon peak (see Fig.~\ref{fig:tof}). With a minimum of 5 \si{\MeV} energy deposition, one already has a signal-to-background ratio of almost $3$. This could be optimized depending the desired physics channel.
\begin{figure}[tb]
	\centering
		\includegraphics[width=0.48\textwidth]{tofm-performance.pdf}
	\caption{Time-of-flight spectrum per meter of BAND hits in coincidence with electrons measured in CLAS12. Clearly a photon and neutron peak is visible over a flat background from accidentals. The dashed lines indicate the range of neutrons with momenta between $200$ and $600$ \si{\MeV/\clight}.}
	\label{fig:tof}
\end{figure}
...
\begin{figure}[tb]
	...
	\caption{Signal counts (blue) and accidental background counts (red) in the neutron signal region of $200$ and $600$ \si{\MeV/\clight}. The signal-to-background ratio (green) shown with a proposed minimum energy deposition cut of 5 \si{\MeV}.}
